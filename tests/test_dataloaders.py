...
def test_local_dataloader(capfd):
    ...
    try:
        ...
        pytest.fail(
            "LocalDataLoader raised StopIteration immediately without yielding."
        )
    ...
    assert set(batch.keys()) == expected_keys, (
        f"Batch keys mismatched. Expected {expected_keys}, got {set(batch.keys())}"
    )

    # Check that shapes conform to expectations (window_size=10, loss_horizon=1)
    assert batch["images"].shape[0] == 10
    assert batch["proprio"].shape[0] == 10
