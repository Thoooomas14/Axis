...
def test_requirements_vs_codebase():
    ...
    assert not missing_deps, (
        f"The following imports are used in code but missing from requirements.txt: {missing_deps}"
    )
...
def test_environment_vs_requirements():
    ...
    assert not missing_in_env, (
        f"packages in requirements.txt but missing from environment.yml: {missing_in_env}"
    )
