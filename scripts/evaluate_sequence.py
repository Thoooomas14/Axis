...
def evaluate_sequence(args):
    ...
    print(f"GT Action (Step 0)  | Mean: {np.mean(np.abs(gt_act_np[:, 0])):6f} | Max: {np.max(np.abs(gt_act_np[:, 0])):6f}")
    print(f"Pred Action (Step 0)| Mean: {np.mean(np.abs(pred_act_np[:, 0])):6f} | Max: {np.max(np.abs(pred_act_np[:, 0])):6f}")
    print("-------------------------\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--random_weights', action='store_true', help="Initialize model with random weights (no checkpoint)")
