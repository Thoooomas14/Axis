...
def verify_model():
    ...
    try:
        ...
        assert action.shape == expected_action_shape, f"Expected Action {expected_action_shape}, got {action.shape}"
        
        assert requery.shape == (B, W, 1) or requery.shape == (B, 1), f"Unexpected Requery shape {requery.shape}"
        ...
        # Check Angular (0:3)
        assert (safe_actions[..., 0:3].abs() <= max_angular + 1e-4).all(), "Angular velocity not clamped!"
        # Check Linear (3:6)
        assert (safe_actions[..., 3:6].abs() <= max_linear + 1e-4).all(), "Linear velocity not clamped!"
        
        print(f"  Max Angular: {safe_actions[..., 0:3].abs().max().item()} (Limit: {max_angular})")
        print(f"  Max Linear: {safe_actions[..., 3:6].abs().max().item()} (Limit: {max_linear})")
