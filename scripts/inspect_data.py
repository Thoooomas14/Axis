...
def analyze_positions(all_positions, all_grippers, all_position_deltas, all_twists=None):
...
def inspect_hdf5(data_path: str, num_episodes: int = 10):
    ...
    with h5py.File(data_path, 'r') as f:
        ...
        for i, ep_key in enumerate(episode_keys[:episodes_to_check]):
            ...
            print(f"  Pos X: [{positions[:, 0].min():.4f}, {positions[:, 0].max():.4f}]")
            print(f"  Pos Y: [{positions[:, 1].min():.4f}, {positions[:, 1].max():.4f}]")
            print(f"  Pos Z: [{positions[:, 2].min():.4f}, {positions[:, 2].max():.4f}]")
...
def inspect_streaming(dataset: str, num_windows: int = 100, data_dir: str = None):
    ...
    for i, batch in enumerate(loader):
        ...
        if i < 3:
            ...
            print(f"    Position delta:  {np.linalg.norm(positions[-1] - positions[0]):.6f}")
            print(f"    Twist (action):  [{actions[0, 0]:.6f}, {actions[0, 1]:.6f}, {actions[0, 2]:.6f}, {actions[0, 3]:.6f}, {actions[0, 4]:.6f}, {actions[0, 5]:.6f}]")
...
def main():
    parser = argparse.ArgumentParser(description='Inspect data to understand position scales')
