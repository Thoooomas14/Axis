## This file was copied from the droid dataset builder repository, which is licensed under the MIT License. The original file can be found at
## SOURCE: https://github.com/kpertsch/droid_dataset_builder/blob/main/droid/droid_utils.py
import glob
import os
import cv2
import h5py
import imageio
import numpy as np
import tempfile  # Added for create_video_file
...
class MP4Reader:
    ...
    def get_frame_resolution(self):
        width = self._mp4_reader.get(cv2.CAP_PROP_FRAME_WIDTH)
        height = self._mp4_reader.get(cv2.CAP_PROP_FRAME_HEIGHT)
        return (width, height)

    def get_frame_count(self):
        if self.skip_reading:
            return 0
        frame_count = int(self._mp4_reader.get(cv2.CAP_PROP_FRAME_COUNT))
        return frame_count
