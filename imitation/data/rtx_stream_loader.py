...
def _episode_worker(
    ...
    def compute_twist(pose_curr, pose_next):
        ...
        T_curr_pp = pp.from_matrix(
            torch.tensor(T_curr, dtype=torch.float32).unsqueeze(0),
            ltype=pp.SE3_type,
            check=False,
        )
        T_next_pp = pp.from_matrix(
            torch.tensor(T_next, dtype=torch.float32).unsqueeze(0),
            ltype=pp.SE3_type,
            check=False,
        )
        ...
        for episode in ds:
            ...
            if ram_used_gb > ram_usage_limit:
                if len(episode_buffer) == 0:
                    raise MemoryError(
                        f"System RAM ({ram_used_gb:.1f}GB) exceeds limit ({ram_usage_limit}GB) "
                        f"before loading any episodes! Increase limit or free memory."
                    )
            ...
            # Object Properties
            from imitation.data.goal_oracle import (
                extract_object_properties as extract_props,
            )
            ...
            for i in range(seq_len):
                ...
                if seg:
                    ...
                    goals[i] = oracle.encode_goal(
                        0, props[i], props[i], object_props
                    ).numpy()
                    ...
                    result_queue.put(
                        {
                            "images": ep["images"][
                                w_start:w_end
                            ],  # NumPy (W, C, H, W) Unnormalized 0-255, normalization happens in Train loop for loader
...
class RTXStreamLoader(IterableDataset):
    ...
    def __init__(
        ...
        random_frames=False,
    ):  # random_frames is deprecated but kept for backwards compatibility in init
    ...
    def _init_builder(self):
        ...
        if (
            self.data_dir
            and self.data_dir.startswith("gs://")
            and "fractal" in self.dataset_name
        ):
            ...
            self.builder = tfds.builder(
                self.dataset_name, data_dir=self.data_dir, try_gcs=use_gcs
            )
    ...
    def estimate_total_windows(self, avg_episode_length=100):
        ...
        if num_episodes == 0:
            return None
        usable_steps = max(
            0, avg_episode_length - self.window_size - self.loss_horizon + 1
        )
    ...
    def _start_worker(self, ctx, result_queue, stop_event, num_workers=1):
        ...
        # Always return the first worker; currently only single worker is supported
        ...
        try:
            while True:
                ...
                if "error" in data:
                    ...
                    if restart_count > max_restarts:
                        raise RuntimeError(
                            f"Worker error after max restarts: {data['error']}"
                        )
                    worker.terminate()
                    worker.join(timeout=2.0)
                    if next_worker is not None:
                        if next_stop_event is not None:
                            next_stop_event.set()
                ...
                if "prestart" in data:
                    if next_worker is None:
                        ...
                        next_worker = self._start_worker(
                            ctx, next_queue, next_stop_event
                        )
                    continue
                ...
                yield {
                    ...
                    "target_poses": torch.tensor(
                        data["target_poses"], dtype=torch.float32
                    ),
                    # Expose critical values for vision pre-training
                    "subtask_end_pose": torch.tensor(
                        data["subtask_end_pose"], dtype=torch.float32
                    ),
                    "object_props": torch.tensor(
                        data["object_props"], dtype=torch.float32
                    ),
                }
    ...
    def _get_pose(self, obs) -> np.ndarray:
        """Extract 13D pose: [R_flat(9), position(3), gripper(1)]."""
        pos = np.zeros(3, dtype=np.float32)
        quat = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32)
        rotmat = None
    ...
    def _compute_twist(
        self, pose_curr: np.ndarray, pose_next: np.ndarray
    ) -> np.ndarray:
        """Compute twist from 13D poses using PyPose: ξ = log(T_curr⁻¹ · T_next)."""
        R_curr = pose_curr[:9].reshape(3, 3)
        p_curr = pose_curr[9:12]
    ...
    def _segment_subtasks(self, props: np.ndarray, object_props: dict | None = None):
        ...
        for i in range(len(boundaries) - 1):
            ...
            goal = self.oracle.encode_goal(
                task_type, start_pose, end_pose, object_props
            )
            segments.append(
                {
                    "start": start,
    ...
    def _iter_inprocess(self):
        ...
        read_config = tfds.ReadConfig(
            ...
            else None,  # Sequential if not shuffling
            ...
            for w in range(num_windows):
                ...
                yield {
                    ...
                    "actions": torch.tensor(
                        np.array(target_twists), dtype=torch.float32
                    ),
                    "target_poses": torch.tensor(
                        np.array(target_poses), dtype=torch.float32
                    ),
                    # Expose critical values for vision pre-training
                    "subtask_end_pose": torch.tensor(
                        self._get_goal_for_frame(
                            segments, current_frame, key="end_pose"
                        ),
                        dtype=torch.float32,
                    )
                    if segments
                    else torch.tensor(w_props[-1], dtype=torch.float32),
